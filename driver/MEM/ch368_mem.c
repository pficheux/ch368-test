 ...
 * Event handler
 */
irqreturn_t ch368_mem_irq_handler(int irq, void *dev_id)
{
...
static int ch368_mem_open(struct inode *inode, struct file *file)
{
  ...
  printk(KERN_WARNING "ch368_mem: minor %d not found\n", minor);
...
static int ch368_mem_probe(struct pci_dev *dev, const struct pci_device_id *ent)
{
  ...
  printk(KERN_INFO "ch368_mem: found %04x:%04x\n", ent->vendor, ent->device);
  printk(KERN_INFO "ch368_mem: using major %d and minor %d for this device\n", major, minor);
  ...
  if (data == NULL) {
    printk(KERN_WARNING "ch368_mem: unable to allocate private structure\n");
  ...
  if (ret < 0) {
    printk(KERN_WARNING "ch368_mem: unable to initialize PCI device\n");
  ...
  if (ret < 0) {
    printk(KERN_WARNING "ch368_mem: unable to reserve PCI resources\n");
  ...
  for (i = 0; i < DEVICE_COUNT_RESOURCE; i++) {
    ...
    if (pci_resource_flags(dev, i) & IORESOURCE_MEM) {
      ...
      if (data->mmio == NULL) {
	printk(KERN_WARNING "ch368_mem: unable to remap BAR %d\n", i);
	goto cleanup_pci_iomap;
      }

      data->mmio_len = pci_resource_len(dev, i);

      printk(KERN_INFO "ch368_mem: BAR %d has been remaped at 0x%p\n", i, data->mmio);
  ...
  if (dev->pin) {
    if (pci_enable_msi (dev))
      printk(KERN_WARNING "ch368_mem: unable to init MSI !\n");
    else 
      ret = request_irq(dev->irq, ch368_mem_irq_handler, 0, "ch368_mem", data);
    if (ret < 0) {
      printk(KERN_WARNING "ch368_mem: unable to register irq handler\n");
      goto cleanup_irq;
    }
    else
      printk(KERN_WARNING "ch368_mem: IRQ %d registered !\n", dev->irq);
  }
  else
    printk(KERN_INFO "ch368_mem: no IRQ!\n");
#endif
  ...
  if (IS_ERR(device)) {
    ret = PTR_ERR(device);
    printk(KERN_WARNING "ch368_mem: can't create device %d\n", data->minor);
    goto cleanup_irq;
  }
...
static void ch368_mem_remove(struct pci_dev *dev)
{
  ...
  printk(KERN_INFO "ch368_mem: device removed\n");
}
...
static int __init ch368_mem_init(void)
{
  ...
  if (ret < 0) {
    printk(KERN_WARNING "ch368_mem: unable to get a major\n");
  ...
  if (IS_ERR(ch368_mem_class)) {
    ...
    printk(KERN_WARNING "ch368_mem: can't create class !\n");
  ...
  if (ret < 0) {
    printk(KERN_WARNING "ch368_mem: unable to register PCI driver\n");
